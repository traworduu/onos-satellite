 ...
 */
@Component(immediate = true, service = SatelliteService.class)
public class SatelliteManager implements SatelliteService {

    private static final String APP_NAME = "org.onosproject.satellite";
    ...
    protected SatelliteStore satelliteStore;
    ...
    public void setTargetPortPower(DeviceId deviceId, PortNumber portNumber, double power) {
        ...
        if (powerConfig != null) {
            satelliteStore.setTargetPower(deviceId, portNumber, power);
    ...
    public Double getTargetPortPower(DeviceId deviceId, PortNumber portNumber) {
        ...
        Double power = satelliteStore.getTargetPower(deviceId, portNumber);
        if (power == null) {
    ...
    public Double syncTargetPortPower(DeviceId deviceId, PortNumber portNumber) {
        ...
        if (powerConfig != null) {
            ...
            if (pl.isPresent()) {
                satelliteStore.setTargetPower(deviceId, portNumber, pl.get());
                return pl.get();
            } else {
                satelliteStore.removeTargetPower(deviceId, portNumber);
            }
        }
        return null;
    }
    ...
    private void initDevices() {
        for (Device device : deviceService.getDevices(Device.Type.ROADM)) {
            ...
            //FIXME
            // As satellite application is a optional tool for now.
    ...
    // Initialize SatelliteStore for a device to support target power
    private void initDevice(DeviceId deviceId) {
        if (!satelliteStore.deviceAvailable(deviceId)) {
            satelliteStore.addDevice(deviceId);
        }
    ...
    private void setAllInitialTargetPortPowers(DeviceId deviceId) {
        ...
        for (Port port : ports) {
            ...
            if (range.isPresent()) {
                Double power = satelliteStore.getTargetPower(deviceId, port.number());
                if (power == null) {
                    // Set default to middle of the range
                    power = (range.get().lowerEndpoint() + range.get().upperEndpoint()) / 2;
                    satelliteStore.setTargetPower(deviceId, port.number(), power);
                }
