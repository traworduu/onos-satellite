 ...
 */
public class SatelliteFlowViewMessageHandler extends UiMessageHandler {

    private static final String ROADM_FLOW_DATA_REQ = "satelliteFlowDataRequest";
    private static final String ROADM_FLOW_DATA_RESP = "satelliteFlowDataResponse";
    private static final String ROADM_FLOWS = "satelliteFlows";

    private static final String ROADM_SET_ATTENUATION_REQ = "satelliteSetAttenuationRequest";
    private static final String ROADM_SET_ATTENUATION_RESP = "satelliteSetAttenuationResponse";

    private static final String ROADM_DELETE_FLOW_REQ = "satelliteDeleteFlowRequest";

    private static final String ROADM_CREATE_FLOW_REQ = "satelliteCreateFlowRequest";
    private static final String ROADM_CREATE_FLOW_RESP = "satelliteCreateFlowResponse";

    private static final String ROADM_SHOW_ITEMS_REQ = "satelliteShowFlowItemsRequest";
    private static final String ROADM_SHOW_ITEMS_RESP = "satelliteShowFlowItemsResponse";
    ...
    private final class FlowTableDataRequestHandler extends TableRequestHandler {
        ...
        private String getCurrentPower(DeviceId deviceId, ChannelData channelData) {
            if (hasAttenuation(deviceId, channelData)) {
                ...
                Double currentPower = satelliteService.getCurrentChannelPower(deviceId,
            ...
            Double currentPower = satelliteService.getCurrentPortPower(deviceId, port);
            return SatelliteUtil.objectToString(currentPower, SatelliteUtil.UNKNOWN);
        }
        ...
        private String getAttenuation(DeviceId deviceId, ChannelData channelData) {
            ...
            Double attenuation = satelliteService.getAttenuation(deviceId, channelData.outPort(), signal);
            return SatelliteUtil.objectToString(attenuation, SatelliteUtil.UNKNOWN);
        }
        ...
        private boolean hasAttenuation(DeviceId deviceId, ChannelData channelData) {
            ...
            return satelliteService.attenuationRange(deviceId, channelData.outPort(), signal) != null;
        }
    }
    ...
    private final class SetAttenuationRequestHandler extends RequestHandler {
        ...
        public void process(ObjectNode payload) {
            ...
            OchSignal signal = channelData.ochSignal();
            Range<Double> range = satelliteService.attenuationRange(deviceId, port, signal);
            Double attenuation = payload.get(ATTENUATION).asDouble();
            boolean validAttenuation = range != null && range.contains(attenuation);
            if (validAttenuation) {
                satelliteService.setAttenuation(deviceId, port, signal, attenuation);
            }
            ...
            rootNode.put(SatelliteUtil.VALID, validAttenuation);
            if (range == null) {
                rootNode.put(SatelliteUtil.MESSAGE, NO_ATTENUATION_MSG);
            } else {
                rootNode.put(SatelliteUtil.MESSAGE, String.format(ATTENUATION_RANGE_MSG, range.toString()));
            }
    ...
    private final class DeleteConnectionRequestHandler extends RequestHandler {
        ...
        public void process(ObjectNode payload) {
            ...
            satelliteService.removeConnection(deviceId, flowId);
        }
    }
    ...
    private final class CreateConnectionRequestHandler extends RequestHandler {
        ...
        public void process(ObjectNode payload) {
            ...
            boolean validInPort = satelliteService.validInputPort(did, inPort);
            boolean validOutPort = satelliteService.validOutputPort(did, outPort);
            boolean validConnect = satelliteService.validConnection(did, inPort, outPort);
            boolean validSpacing = true;
            boolean validChannel = satelliteService.validChannel(did, inPort, och);
            boolean channelAvailable = satelliteService.channelAvailable(did, och);
            boolean validAttenuation = satelliteService.attenuationInRange(did, outPort, att);

            if (validConnect) {
                if (validChannel && channelAvailable) {
                    if (validAttenuation) {
                        satelliteService.createConnection(did, priority, permanent, timeout, inPort, outPort, och, att);
                    } else {
                        satelliteService.createConnection(did, priority, permanent, timeout, inPort, outPort, och);
                    }
                }
            }
            ...
            if (showItems) {
                ...
                if (!validChannel) {
                    Set<OchSignal> lambdas = satelliteService.queryLambdas(did, outPort);
                    if (lambdas != null) {
                ...
                if (!validAttenuation) {
                    Range<Double> range = satelliteService.attenuationRange(did, outPort, och);
                    if (range != null) {
        ...
        private ObjectNode validationObject(boolean result, String message) {
            ObjectNode node = objectNode();
            node.put(SatelliteUtil.VALID, result);
            if (!result) {
                // return error message to display if validation failed
                node.put(SatelliteUtil.MESSAGE, message);
            }
            return node;
        }
